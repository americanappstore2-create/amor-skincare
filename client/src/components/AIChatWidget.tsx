  ...
  return (
    <>
        ...
        <div className="fixed bottom-24 right-4 md:right-6 z-50 w-[340px] md:w-[380px] animate-scale-in">
          ...
          >
            <div className="px-5 py-4 flex items-center gap-3 bg-[#1a1a1a]">
              ...
              <button onClick={() => setIsOpen(false)} className="h-7 w-7 flex items-center justify-center text-white/50 hover:text-white transition-colors">
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#faf7f4]">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} animate-fade-in`}>
                  {msg.role === "assistant" && (
                    <img src={LOGO_URL} alt="" className="h-6 w-6 rounded-full object-contain bg-white mr-2 shrink-0 mt-0.5" style={{ border: "1px solid #e8e0d8" }} />
                  )}
                  <div
                    className="max-w-[80%] px-3.5 py-2.5 text-[13px] leading-relaxed font-light"
                    style={
                      msg.role === "user"
                        ? { background: "#1a1a1a", color: "white", borderRadius: "1rem 1rem 0.25rem 1rem" }
                        : { background: "white", color: "#1a1a1a", border: "1px solid #e8e0d8", borderRadius: "1rem 1rem 1rem 0.25rem" }
                    }
                  >
                    {msg.content}
                  </div>
              ...
              {isTyping && (
                <div className="flex justify-start animate-fade-in">
                  <img src={LOGO_URL} alt="" className="h-6 w-6 rounded-full object-contain bg-white mr-2 shrink-0" style={{ border: "1px solid #e8e0d8" }} />
                  <div className="px-4 py-3" style={{ background: "white", border: "1px solid #e8e0d8", borderRadius: "1rem 1rem 1rem 0.25rem" }}>
                    <div className="flex gap-1">
                      {[0, 150, 300].map((delay) => (
                        <div key={delay} className="h-1.5 w-1.5 rounded-full bg-[#c9a96e] animate-bounce" style={{ animationDelay: `${delay}ms` }} />
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <form onSubmit={handleSubmit} className="p-3 bg-white" style={{ borderTop: "1px solid #e8e0d8" }}>
              <div className="flex gap-2">
                ...
                <button type="submit" disabled={!input.trim() || isTyping}
                  ...
                  {isTyping ? <Loader2 className="h-4 w-4 text-white animate-spin" /> : <Send className="h-4 w-4 text-white" />}
                </button>
              </div>
            </form>
          </div>
      ...
      <button
        ...
        style={{ background: "#1a1a1a", boxShadow: "0 8px 24px rgba(0,0,0,0.25)" }}
        aria-label="Открыть чат"
      >
        {isOpen ? <X className="h-5 w-5" /> : <MessageCircle className="h-5 w-5" />}
        {!isOpen && (
          <span className="absolute -top-1 -right-1 h-4 w-4 rounded-full flex items-center justify-center" style={{ background: "#c9a96e", border: "2px solid white" }}>
